...
This module provides an interface to the Clang indexing library.
It is a low-level interface to the indexing library which
attempts to match the Clang API directly while also being
"pythonic".  Notable differences from the C API are:

 * string results are returned as Python strings, not CXString
   objects.

 * null cursors are translated to None.

 * access to child cursors is done via iteration, not
   visitation.
...
Most object information is exposed using properties, when the
underlying API call is efficient.
"""
# TODO
# ====
#
# o API support for invalid translation units. Currently we can't even
#   get the diagnostics on failure because they refer to locations in
#   an object that will have been invalidated.
#
# o fix memory management issues (currently client must hold on to
#   index and translation unit, or risk crashes).
#
# o expose code completion APIs.
#
# o cleanup ctypes wrapping, would be nice to separate the ctypes
#   details more clearly, and hide from the external interface (i.e.,
#   help(cindex)).
#
...
# Python 3 strings are unicode, translate them to/from utf8 for
# C-interop.
class c_interop_string(ctypes.c_char_p):
    def __init__(self, p:Union[str, bytes, None]=None) -> None:
        if p is None:
            p = b""
        if isinstance(p, str):
            p = p.encode("utf8")
    ...
    @classmethod
    def from_param(cls, param:Optional[Union[str, bytes]]) -> Optional['c_interop_string']: # type: ignore
...
class TranslationUnitLoadError(Exception):
    ...
    This is raised in the case where a TranslationUnit could not
    be instantiated due to failure in the libclang library.

    FIXME: Make libclang expose additional error information in
    this scenario.
    """
    pass
...
class TranslationUnitSaveError(Exception):
    ...
    Each error has associated with it an enumerated value,
    accessible under e.save_error. Consumers can compare the
    value with one of the ERROR_ constants in this class.
    """

    # Indicates that an unknown error occurred. This typically
    # indicates that I/O failed during save.
    ERROR_UNKNOWN = 1

    # Indicates that errors during translation prevented saving.
    # The errors should be available via the TranslationUnit's
    # diagnostics.
...
class CachedProperty(Generic[T]):
    ...
    The first time the property is accessed, the original
    property function is executed. The value it returns is set
    as the new value of that instance's property, replacing the
    original method.
    """

    def __init__(self, wrapped:Callable[[Any], T]) -> None:
    ...
    def __get__(self, instance:Any, instance_type=None) -> T:
        if instance is None:
            #FIXME
...
class SourceLocation(ctypes.Structure):
    ...
    def _get_instantiation(self):
        if self._data is None:
            ...
            conf.lib.clang_getInstantiationLocation(self, ctypes.byref(f), ctypes.byref(l),
                    ctypes.byref(c), ctypes.byref(o))
            if f:
                f = File(f)
            else:
                f = None
...
class TokenGroup(object):
    ...
    @staticmethod
    def get_tokens(tu, extent) -> Iterator[Token]:
        """
        ...
        tokens_array = ctypes.cast(tokens_memory, ctypes.POINTER(Token * count)).contents
...
class TokenKind:
    ...
    def __init__(self, value:int, name:str) -> None:
        """
        Create a new TokenKind instance from a numeric value and
        a name.
        """
        self.value = value
        self.name = name
    ...
    def register(value:int, name:str) -> None:
        ...
        This should only be called at module load time by code
        within this package.
        """
...
BE = TypeVar('BE', bound='BaseEnumeration')
class BaseEnumeration:
    _kinds : Sequence[Optional['BaseEnumeration']]
    _name_map : Dict['BaseEnumeration', 'str']
    _kind_map : Dict['str', 'BaseEnumeration']
    @classmethod
    def kind(cls:TypingType[BE], name:str) -> BE:
        if name not in cls._kind_map:
            raise KeyError(name)
        result: BE = cls._kind_map[name] # type: ignore
        return result
    ...
    def __init_subclass__(cls, table) -> None:
        ...
        cls._name_map = {}
        cls._kind_map = {}
        cls.register_kinds(table)
    def __init__(self, value:int, name:str) -> None:
        kinds: List[Optional['BaseEnumeration']] = self._kinds # type: ignore
        if value >= len(kinds):
            kinds.extend([None] * (value - len(kinds)+1))
        if kinds[value] is not None:
            raise ValueError(f'{self.__class__} value {value} already loaded')
        self.value = value
        self.name = name
        kinds[value] = self

    @classmethod
    def register(cls:TypingType[BE], value:int, name:str) -> BE:
        v = cls(value, name)
        cls._name_map[v] = name
        cls._kind_map[name] = v
        return v
    ...
    @classmethod
    def from_id(cls:TypingType[BE], id:int) -> BE:
        return cls._kinds[id] # type: ignore

    def __repr__(self) -> str:
        return f'{self.__class__}.{self.name}'

    @classmethod
    def register_kinds(cls, kinds) -> None:
        for name, value in kinds:
            setattr(cls, name, cls.register(value, name))



### Cursor Kinds ###
class CursorKind(BaseEnumeration, table=enumerations.CURSOR_KINDS):
    _kinds : Sequence[Optional['CursorKind']]
    """
    A CursorKind describes the kind of entity that a cursor
    points to.
    """
...
class TemplateArgumentKind(BaseEnumeration, table=enumerations.TEMPLATE_ARGUMENT_KINDS):
    """
    A TemplateArgumentKind describes the kind of entity that a template argument
    represents.
    """
    _kinds : Sequence[Optional['TemplateArgumentKind']]

### Exception Specification Kinds ###
class ExceptionSpecificationKind(BaseEnumeration, table=enumerations.EXCEPTION_SPECIFICATIONS):
    """
...
class Cursor(ctypes.Structure):
    ...
    def get_num_template_arguments(self) -> ctypes.c_int:
        """
    ...
    def get_template_argument_type(self, num:int) -> TemplateArgumentKind:
        """
        Returns the CXType for the indicated template argument.
        """
        return conf.lib.clang_Cursor_getTemplateArgumentType(self, num)

    def get_template_argument_value(self, num:int) -> ctypes.c_longlong:
        """
    ...
    def get_template_argument_unsigned_value(self, num:int) ->ctypes.c_ulonglong:
        """
    ...
    def get_bitfield_width(self) -> bool:
        """
...
class StorageClass(BaseEnumeration, table=enumerations.STORAGE_CLASSES):
    """
    Describes the storage class of a declaration
    """
### Availability Kinds ###

class AvailabilityKind(BaseEnumeration, table=enumerations.AVAILABILITY_KINDS):
    """
...
class AccessSpecifier(BaseEnumeration, table=enumerations.ACCESS_SPECIFIERS):
    """
...
class TypeKind(BaseEnumeration, table=enumerations.TYPE_KINDS):
    """
...
class RefQualifierKind(BaseEnumeration, table=enumerations.REF_QUALIFIER_KINDS):
    """
    Describes a specific ref-qualifier of a type.
    """

class LinkageKind(BaseEnumeration, table=enumerations.LINKAGE_KINDS):
    """
    Describes the kind of linkage of a cursor.
    """

class TLSKind(BaseEnumeration, table=enumerations.TLS_KINDS):
    """
...
class _CXUnsavedFile(ctypes.Structure):
    ...
    """
    _fields_ = [("name", ctypes.c_char_p), ("contents", ctypes.c_char_p), ('length', ctypes.c_ulong)]

# Functions calls through the python interface are rather slow.
# Fortunately, for most symbols, we do not need to perform a function
# call. Their spelling never changes and is consequently provided by this
# spelling cache.
SpellingCache = {
...
class CompletionChunk:
    ...
    # We do not use @CachedProperty here, as the manual implementation
    # is apparently still significantly faster. Please profile
    # carefully if you would like to add CachedProperty back.
    @property
...
class CompletionString(ClangObject):
    ...
    def __repr__(self) -> str:
        return (" | ".join([str(a) for a in self]) # type: ignore
...
class CCRStructure(ctypes.Structure):
    _fields_ = [('results', ctypes.POINTER(CodeCompletionResult)),
...
class CompilationDatabaseError(Exception):
    ...
    def __init__(self, enumeration:int, message):
        # assert isinstance(enumeration, int)
...
class Token(ctypes.Structure):
    ...
    def cursor(self):
        ...
        conf.lib.clang_annotateTokens(self._tu, ctypes.byref(self), 1, ctypes.byref(cursor))
...
callbacks['translation_unit_includes'] = ctypes.CFUNCTYPE(None, c_object_p,
        ctypes.POINTER(SourceLocation), ctypes.c_uint, ctypes.py_object)
callbacks['cursor_visit'] = ctypes.CFUNCTYPE(ctypes.c_int, Cursor, Cursor, ctypes.py_object)
...
functionList = [
  ...
  ("clang_codeCompleteAt",
   [TranslationUnit, c_interop_string,ctypes.c_int,ctypes.c_int, ctypes.c_void_p,ctypes.c_int,ctypes.c_int],
...
#  [CXTUResourceUsage]),
  ...
  ("clang_disposeTokens",
   [TranslationUnit, ctypes.POINTER(Token),ctypes.c_uint]),
...
#  CXTUResourceUsage),
  ...
  ("clang_getInstantiationLocation",
   [SourceLocation, ctypes.POINTER(c_object_p), ctypes.POINTER(ctypes.c_uint), ctypes.POINTER(ctypes.c_uint),
  ...
  ("clang_tokenize",
   [TranslationUnit, SourceRange, ctypes.POINTER(ctypes.POINTER(Token)), ctypes.POINTER(ctypes.c_uint)]),
...
def register_function(lib:ctypes.CDLL, item, ignore_errors:bool) -> None:
    # A function may not exist, if these bindings are used with an
    # older or incompatible version of libclang.so.
    try:
...
def register_functions(lib:ctypes.CDLL, ignore_errors:bool) -> None:
    """
    Register function prototypes with a libclang library instance.

    This must be called as part of library instantiation so Python
    knows how to call out to the shared library.
    """
...
class Config:
    ...
    def set_compatibility_check(check_status:bool) -> None:
        ...
        The python bindings are only tested and evaluated with the
        version of libclang they are provided with. To ensure correct
        behavior a (limited) compatibility check is performed when
        loading the bindings. This check will throw an exception, as
        soon as it fails.

        In case these bindings are used with an older version of
        libclang, parts that have been stable between releases may
        still work. Users of the python bindings can disable the
        compatibility check. This will cause the python bindings to
        load, even though they are written for a newer version of
        libclang. Failures now arise if unsupported or incompatible
        features are accessed. The user is required to test themselves
        if the features they are using are available and compatible
