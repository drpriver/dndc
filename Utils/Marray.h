...
#define MARRAY_FOR_EACH(iter, marray) for(typeof((marray).data[0]) *iter = (marray).data, *iter##end__=(marray).data?((marray).data+(marray).count):NULL; iter != iter##end__; ++iter)
#endif
