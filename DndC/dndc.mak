...
$(BINDIR)/dndcbench$(EXE): DndC/dndc.c $(DEPDIR)/dndcbench.dep DndC/dndc.mak $(OBJDIR)/frozenstdlib.o
	$(CC) $(FLAGS) $(PLATFORM_FLAGS) -O1 -g $(PYCFLAGS) $(DEPFLAGS) $(DEPDIR)/dndcbench.dep $< $(OBJDIR)/frozenstdlib.o -o $@ $(LINK_FLAGS) $(PYLDFLAGS) -DBENCHMARKING -DBENCHMARKINPUTPATH='"$(BENCHMARKINPUTPATH)"' -DBENCHMARKOUTPUTPATH='"$(BENCHMARKOUTPUTPATH)"' -DBENCHMARKDIRECTORY='"$(BENCHMARKDIRECTORY)"' -DBENCHMARKITERS=$(BENCHMARKITERS) -DDNDCMAIN
